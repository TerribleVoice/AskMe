...
export const Auth = () => {
  ...
  const changeInput = (event: React.ChangeEvent<HTMLInputElement>) => {
    //чтобы записать из формы в переменные
    event.persist();
    setRegister((prev) => {
      return {
        ...prev,
  ...
  //   //объявление функции для передачи в форму типо по нажатию кнопки
  //   let isAuth = document
  //     .querySelector(".left-reg__mail")
  //     .classList.contains("hide"); //переключение класса для показа
  //   if (isAuth) {
  //     axios({
  //       //библа для нодджс для http запросов, можно юзать потом для других штук в проекте
  ...
  //     axios({
  //       //вызов метода апи, можно юзать потом для других штук в проекте
  //       method: "post",
  ...
  //       },
  //       data: {
  //         //возможно регистрация не работает из-за обновленного бека, там больше полей для данных, не все параметры передаются(смотреть свагер)
  ...
  //     })
  //       .then((res) => {
  //         //обработка результата для авторизации и регистрации, рес=ответ от сервера
  //       })
  //       .catch((err) => alert(err));
  //   }
        ...
        <div className="reg">
          <div className="reg__left left-reg">
            <div className="left-reg__choose">
              ...
              <div className="left-reg__auth" onClick={onAuthClick}>
                Авторизация
              </div>
            </div>
